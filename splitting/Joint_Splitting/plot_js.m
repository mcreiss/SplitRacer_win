function plot_js(results,station,nc,sel_data)

% plot joinst splitting results for one layer
...
joint_split_fig = figure('Name',...
    'Joint Splitting Single Layer - SplitRacer 2.0','NumberTitle','off',...
...
%% plot histogram
pause(0.1)
draw_bars_js(results.ic_phi, results.ic_delta, results.fa_area,hsp);

%% plot confidence level
pause(0.1)
ax2 = axes('Position',[0.2,0.05,0.55,0.4],'Parent',hsp);

Plot95Conf(results.maxtime, results.Ematrix, ...
    results.level, results.phi,results.delta, results.fa_area,ax2);
