...
for i=1:length(selected_stations)
    ...
    for ii = 1:length(ana_phases)
        ...
        tab(ii) = uitab(tgroup,'Title',['Event ', num2str(ii)]);
        tgroup.SelectedTab = tab(ii);
        plot_si_sp(results.(name_event),add_info,tab(ii));
    ...
    for ii = 1:length(ana_phases)
        ...
        fn2 = ana_phases{ii};
        tgroup.SelectedTab = tab(ii);
        ui_si_split(results.(fn2),add_info,tab(ii),fileID1,dir_save)
